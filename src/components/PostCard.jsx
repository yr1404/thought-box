...
function PostCard(
	{ $id, titles, content, status, featuredImage },
	className
) {
	...
	return (
		<Link to={`/post/${$id}`}>
			<div
				className={`w-full border-[1px] min-h-[330px] border-gray-600 rounded-[12px] shadow-md hover:shadow-gray-700 shadow-gray-900 duration-300 gap-2 ${className}`}>
				...
				<div className="text-left px-4 pt-2">
					...
					<h4 className="lg:text-sm mt- text-sm text-[#909090] tracking-tight pb-4 truncate">
						{sanitizedContent}
					</h4>
				</div>
			</div>
